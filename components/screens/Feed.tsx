...
export function Feed({ navigate }: FeedProps) {
  ...
  const getSourceIcon = (type: string) => {
    switch (type) {
      ...
      case 'api':
        return <Code size={16} className="text-[#FF6B6B]" />;
      default:
        return <MessageSquare size={16} className="text-[#FFB800]" />;
    }
  };
    ...
    <div className="h-full flex flex-col bg-[#222831]">
      <div className="flex items-center justify-between px-6 pt-6 pb-4">
        ...
        <motion.button
          ...
          className="bg-[#222831] border-2 border-[#D65A31] text-[#D65A31] px-4 py-2 rounded-full font-semibold flex items-center gap-2 hover:bg-[#D65A31] hover:text-[#EEEEEE] transition-colors"
        >
          <Plus size={20} />
          Create Agent
        </motion.button>
        ...
        {loading && !refreshing ? (
          ...
          >
            <div className="w-32 h-32 mb-6 rounded-full bg-[#393E46] flex items-center justify-center">
              <Newspaper size={48} className="text-[#EEEEEE]/30" />
            </div>
            <h3 className="text-xl font-bold text-[#EEEEEE] mb-2">No updates found</h3>
            ...
            {feedItems.map((item, index) => (
              ...
              >
                <div className="flex items-start gap-3 mb-3">
                  <div className="bg-[#222831] p-2 rounded-xl">
                    {getSourceIcon(item.agent?.source_type || 'subject')}
                  </div>
                  <div className="flex-1 min-w-0">
                    <h3 className="text-[#EEEEEE] font-semibold text-lg truncate mb-1">
                      {item.agent?.title || 'Untitled Agent'}
                    </h3>
                    <div className="flex items-center gap-2 text-xs text-[#EEEEEE]/60">
                ...
                <p className="text-[#EEEEEE]/80 text-sm leading-relaxed mb-4 line-clamp-3">
                  {item.summary}
                </p>
                ...
                <div className="flex gap-2">
                  <motion.button
                    whileTap={{ scale: 0.95 }}
                    className="flex-1 bg-[#222831] text-[#EEEEEE] py-2 px-3 rounded-xl text-sm font-medium flex items-center justify-center gap-2"
                  >
                    <TrendingUp size={16} />
                    View
                  </motion.button>
                  {item.has_analysis && (
                    <motion.button
                      whileTap={{ scale: 0.95 }}
                      className="flex-1 bg-[#222831] text-[#EEEEEE] py-2 px-3 rounded-xl text-sm font-medium"
                    >
                      Analysis
                    </motion.button>
                  )}
                  {item.alert_count > 0 && (
                    <motion.button
                      whileTap={{ scale: 0.95 }}
                      className="bg-[#D65A31] text-[#EEEEEE] py-2 px-3 rounded-xl text-sm font-medium flex items-center gap-2"
                    >
                      <Bell size={16} />
                      {item.alert_count}
                    </motion.button>
